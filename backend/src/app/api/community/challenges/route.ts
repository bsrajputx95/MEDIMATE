...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!userId) return NextResponse.json({ data: allChallenges.map((c: any) => ({ ...c, isJoined: false })) });
    ...
    return NextResponse.json({ data: allChallenges.map((c: any) => ({ ...c, isJoined: joinedIds.has(c.id) })) });  } catch { return NextResponse.json({ message: 'Failed' }, { status: 500 }); }
}
